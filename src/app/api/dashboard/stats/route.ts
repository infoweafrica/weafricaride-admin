...
export async function GET(request: NextRequest) {
  ...
  try {
    /*
     * The admin city selector supplies a UUID.
     * The rides/drivers tables store the city as text in `city`.
     * Resolve the UUID to the actual city name before filtering.
     */
    ...
    if (cityId) {
      ...
      if (cityError) {
        console.error("[/api/dashboard/stats] city lookup failed:", cityError);
        return NextResponse.json(
          { error: "Failed to resolve selected city" },
          { status: 500 }
        );
      }
    ...
    const [
      ...
      (() => {
        let q = supabase.from("rides").select("status");
      ...
      (() => {
        let q = supabase
          .from("rides")
          .select(
            "*, rider:riders(full_name), driver:drivers(full_name), category:ride_categories(name)"
          )
    ...
    const weeklyRevenue = Object.entries(revByDay).map(
      ([name, revenue]) => ({
        name,
        revenue,
      })
    ...
    (weeklyRidesRes.data || []).forEach((r: any) => {
      ...
      if (r.status === "completed") {
        ridesByDay[day].completed++;
      }

      if (cancelledStatuses.includes(r.status)) {
        ridesByDay[day].cancelled++;
      }
    });

    const weeklyRides = Object.entries(ridesByDay).map(([name, v]) => ({
      name,
      ...v,
    }));
