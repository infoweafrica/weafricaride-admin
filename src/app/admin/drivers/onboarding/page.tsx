...
function OnboardingContent() {
  const [allDrivers, setAllDrivers] = useState<OnboardingDriver[]>([]);
  ...
  // admin_approve_driver runs into require_driver_verification_complete
  // (a DB trigger) if date_of_birth is null — nothing in the driver-app
  // onboarding flow ever collects it, so this is the only place it can
  // be filled in before approval is even possible.
  //
  // This must go through /api/drivers/[id] (service-role, session-checked)
  // rather than a direct `supabase.from(...).update()` call — the browser
  // `supabase` client here is a bare anon-key client with no auth context
  // (admin sessions are a separate signed cookie, not a real Supabase
  // JWT), so no RLS policy on `drivers` ever matches it. A direct write
  // fails silently: no error, zero rows affected, DOB stays null.
  const handleSaveDob = async () => {
    ...
    try {
      const res = await fetch(`/api/drivers/${selectedDriver.id}`, {
        ...
        body: JSON.stringify({ date_of_birth: dobInput }),
      });
      const result = await res.json();
      if (!res.ok) throw new Error(result.error || "Failed to save date of birth");
      setSelectedDriver({ ...selectedDriver, date_of_birth: dobInput });
      fetchDrivers();
    } catch (e) {
      alert(e instanceof Error ? e.message : "Failed to save date of birth");
    } finally {
      setSavingDob(false);
    }
  };

  // Always fetched unfiltered by status -- selectedStatus only filters
  // client-side (see displayedDrivers below). Filtering server-side here
  // meant every tab's count (including "All") was computed from whatever
  // subset the currently-selected tab had already narrowed the fetch
  // down to, so every count but the active tab's always read 0.
  ...
  const displayedDrivers = selectedStatus === "all"
    ? allDrivers
    : allDrivers.filter((d) => d.approval_status === selectedStatus);
  ...
  const counts = STATUS_FILTERS.filter((s) => s.value !== "all").reduce(
    (acc, stage) => {
      acc[stage.value] = allDrivers.filter((d) => d.approval_status === stage.value).length;
      return acc;
    },
            ...
            <div className="p-6 space-y-6">
              ...
              <Section title="Identity & Documents">
                {!selectedDriver.date_of_birth && (
                  <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-lg -mt-1">
                ...
                <InfoRow label="Date of Birth" value={selectedDriver.date_of_birth || "—"} />
