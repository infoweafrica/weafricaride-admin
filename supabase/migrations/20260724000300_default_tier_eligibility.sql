-- Replaces exact-tier-only dispatch matching with a fixed default
-- eligibility table per vehicle tier, applied automatically to every
-- driver (no setup needed):
--   go      -> go
--   x       -> go, x
--   xl      -> go, x, xl
--   comfort -> x, comfort   (skips xl — comfort is about quality, not size)
--   black   -> comfort, black
-- drivers.preferred_categories (added in 20260724000200) still layers on
-- top as an optional further opt-in beyond these defaults.
-- Women requests: eligible to any female driver whose car's own tier is
-- go/x/xl/comfort (i.e. any tier except black).
--
-- Idempotent — safe to re-run via `psql -f`.

CREATE OR REPLACE FUNCTION public.default_eligible_ride_tiers(p_vehicle_tier text)
RETURNS text[]
LANGUAGE sql IMMUTABLE
AS $$
  SELECT CASE p_vehicle_tier
    WHEN 'go' THEN ARRAY['go']
    WHEN 'x' THEN ARRAY['go', 'x']
    WHEN 'xl' THEN ARRAY['go', 'x', 'xl']
    WHEN 'comfort' THEN ARRAY['x', 'comfort']
    WHEN 'black' THEN ARRAY['comfort', 'black']
    ELSE ARRAY[]::text[]
  END;
$$;

GRANT EXECUTE ON FUNCTION public.default_eligible_ride_tiers(text) TO anon, authenticated, service_role;
...
BEGIN
  ...
  WHERE (
    ...
    -- Explicit preferences (if the driver has ever saved any) fully
    -- replace the tier default — this lets a driver opt OUT of part of
    -- their own default set (e.g. an X-tier car that tunes off X keeps
    -- only Go), not just opt into extra tiers.
    OR v_ride.vehicle_class = ANY(COALESCE(cd.preferred_categories, public.default_eligible_ride_tiers(cd.tier)))
